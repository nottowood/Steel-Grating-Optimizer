...
| 3 | Expansion Matching Engine | Match stock pieces to expansion demands by manufacturing compatibility |
...
| **Dead Scrap** | A Reduction Strip that fails reusability criteria. No recovery possible. |
| **Expansion Demand** | The additional width required when Fab Width > Standard Width. Width = Fab Width − Standard Width. |
| **Manufacturing Compatibility** | Two pieces are weld-compatible when they share the same Load Bar Pitch AND Load Bar Thickness. This guarantees identical bar spacing and bar cross-section. |
| **Recovery Match** | A pairing of one Stock Piece to one Expansion Demand where the piece is manufacturing-compatible and dimensionally sufficient |
...
## 4. MANUFACTURING COMPATIBILITY MODEL
...
Matching is based on **manufacturing compatibility**, defined by the physical properties that determine weld compatibility:

| Property | Why It Matters |
|----------|---------------|
| **Load Bar Pitch** | Bars must align for welding. Different pitch = bars don't line up. |
| **Load Bar Thickness** | Bar cross-section must match for structural continuity. |
...
```
compatibility_key = (load_bar_pitch, load_bar_thickness)
```

Two panels are manufacturing-compatible if and only if they share the same compatibility key.

### Examples

| Strip Source | Demand Target | Pitch | Thickness | Compatible? |
|-------------|---------------|-------|-----------|-------------|
| TA325/1 | TA325/1 | 60=60 | 25=25 | YES |
| TA325/1 | TB325/1 | 60=60 | 25=25 | YES (same pitch and thickness, different cross bar type) |
| TA325/1 | TA330/1 | 60=60 | 25≠30 | NO (different thickness) |
| TA325/1 | TA225/2 | 60≠40 | 25=25 | NO (different pitch / series) |
...
MATCHING phase
    → Stock Piece consumed for expansion demand
        ...
        → IF remainder meets reusability criteria:
            → Stock piece stays AVAILABLE with reduced remaining_width
        → ELIF remainder > 0:
            → Stock piece status = DEPLETED
            → Create Dead Scrap record for remainder
        → ELSE (exact fit):
            → Stock piece status = DEPLETED
...
StockPiece:
    ...
    load_bar_pitch        — mm (for compatibility matching)
    load_bar_thickness    — mm (for compatibility matching)
    ...
    status                — AVAILABLE | DEPLETED | DEAD_SCRAP
...
|-----------|---------------|
| width > 200 AND length > 200 AND area >= 0.20 m² | REUSABLE → becomes Stock Piece |
...
|------------|------|-----------|
| **C1: Manufacturing Compatible** | stock.load_bar_pitch == demand.load_bar_pitch AND stock.load_bar_thickness == demand.load_bar_thickness | Load bars must align and match cross-section for welding |
...
FOR each expansion demand (sorted by expansion_width descending — largest first):
    ...
    WHILE remaining_demand_qty > 0:
        ...
        IF candidates exist:
            ...
            Classify remainder:
                IF best.remaining_width meets reusability → stays AVAILABLE
                ELIF best.remaining_width > 0 → create TrimWastePiece, mark DEPLETED
                ELSE → mark DEPLETED (exact fit)
        ELSE:
            Mark remaining demand as UNMATCHED
            BREAK
```

### Why Manufacturing Compatibility Instead of Product Code?
...
The physical constraint is bar alignment, not cross bar configuration. Cross bars run perpendicular to the expansion joint and do not need to align.

### Sample Data Expectation

| Demand | Exp Width | Stock Source | Stock Width | C1 (pitch,thick) | C2 | C3 | Match |
|--------|-----------|-------------|-------------|-------------------|----|----|-------|
| G01 (120mm exp) | 120 mm | G04 (390mm stock) | 390 mm | (60,25)=(60,25) YES | 390>=120 YES | 1450>=1450 YES | MATCHED |

After matching:
- G04 stock piece: remaining_width = 390 − 120 = 270mm
- 270mm remainder: width 270 > 200, length 1450 > 200, area 0.3915 >= 0.20 → stays AVAILABLE in inventory
...
RecoveryMatch:
    ...
    trim_classification     — REUSABLE | DEAD_SCRAP (classification of remainder)
...
| R12 | Compatibility Verified | Every match satisfies C1 (pitch + thickness), C2 (width), C3 (length) |
...
| R17 | Dead Scrap Accounted | Dead Scrap from classification + Dead Scrap from trim = Total Dead Scrap |
...
Panel G01 (expansion 120mm, qty=10)
  ...
  → Remainder: 270mm (REUSABLE → stays AVAILABLE)
  → Candidates considered: 1
  → Decision: Only compatible stock piece available
  → Compatibility: pitch=60mm, thickness=25mm
```
...
| `recovery_engine.py` | Stock piece generation, compatibility matching, recovery optimization |
...
class StockPiece:
    ...
    load_bar_pitch: float           # mm — for compatibility key
    load_bar_thickness: float       # mm — for compatibility key
    ...
    def compatibility_key(self) -> tuple:
        return (self.load_bar_pitch, self.load_bar_thickness)
...
class DeadScrapPiece:
    ...
    area_m2: float
    qty: int
    origin: str                     # REDUCTION | TRIM_WASTE
...
class RecoveryMatch:
    ...
    candidate_count: int
    decision_reason: str
    compatibility_key: tuple        # (pitch, thickness)
...
| Compatibility matching across projects | `compatibility_key` (pitch, thickness) is product-agnostic |
...
| Compatibility key | Constraint for which panels can share a stock length |
...
```
Two panels: Mark A (1250mm) and Mark B (1250mm), same compatibility key
Stock length: 6000mm
...
## APPENDIX A: SAMPLE DATA WALKTHROUGH (REV 0.2)

### Input (sample_data.csv)

| Mark | Product | Width | Length | Qty | Exp | Red | Pitch | Thickness |
|------|---------|-------|--------|-----|-----|-----|-------|-----------|
| G01 | TA325/1 | 1115 | 1450 | 10 | 120mm | — | 60 | 25 |
| G02 | TA325/1 | 995 | 1250 | 25 | — | — | 60 | 25 |
| G03 | TA325/1 | 965 | 1250 | 25 | — | 30mm | 60 | 25 |
| G04 | TA325/1 | 605 | 1450 | 10 | — | 390mm | 60 | 25 |
...
Demand: G01, expansion=120mm, length=1450mm, qty=10
  Compatibility key: (60, 25)

Available stock:
  SP-G04-001: width=390mm, length=1450mm, qty=10, key=(60,25)
    C1: (60,25)==(60,25) → PASS
    C2: 390 >= 120 → PASS
    C3: 1450 >= 1450 → PASS
    C4: AVAILABLE → PASS
...
Match: G01 ← SP-G04-001
  ...
  Remainder check: 270>200, 1450>200, 0.3915>=0.20 → REUSABLE → stays AVAILABLE
```

### Step 3: Final Inventory State

| Piece | Status | Original Width | Remaining Width | Area (m²) |
|-------|--------|---------------|-----------------|-----------|
| SP-G04-001 | AVAILABLE | 390 mm | 270 mm | 0.3915 |
| DS-G03-001 | DEAD_SCRAP | 30 mm | — | 0.0375 |

### Step 4: Recovery KPIs
...
| D3 | ~~Same Product Code constraint~~ → Manufacturing Compatibility (pitch + thickness) | Product Code is overly restrictive — TA and TB with same pitch/thickness are weld-compatible | **0.2** |
...
| D6 | New files only — no modifications to existing files except `processor.py` and `app.py` | Minimizes regression risk to frozen baseline | 0.1 |
...
## APPENDIX C: REV 0.1 → REV 0.2 CHANGE SUMMARY
...
DBD_PHASE3_REV0.2 — DRAFT — Awaiting Approval
